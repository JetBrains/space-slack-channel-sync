...
class SlackClient constructor(
    ...
    companion object {
        suspend fun tryCreate(slackTeamId: String): SlackClient? {
            val slackTeam = db.slackTeams.getById(slackTeamId)
            if (slackTeam == null) {
                log.debug("Slack team record not found by slack team id")
                return null
            }

            return slackAppClient(slackTeam)
        }
    }
    ...
    suspend fun getMessageRaw(channelId: String, messageId: String): String? {
        return fetchRaw { accessToken ->
            httpClientForDownloads.get("https://slack.com/api/conversations.history?channel=$channelId&latest=$messageId&inclusive=true&limit=1") {
