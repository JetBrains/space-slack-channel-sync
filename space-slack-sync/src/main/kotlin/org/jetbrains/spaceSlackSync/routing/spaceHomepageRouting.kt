...
fun Routing.spaceHomepageRouting() {
    ...
    get<Routes.GetSlackWorkspaces> {
        runAuthorized { spaceTokenInfo ->
            val teams = db.slackTeams.getForSpaceOrg(spaceTokenInfo.spaceAppInstance.clientId).map { team ->
                ...
                SlackWorkspace(team.id, team.domain, teamName)
            }
    ...
    get<Routes.UrlForAddingSlackTeam> {
        runAuthorized { spaceTokenInfo ->
            log.debug("Adding Slack team to Space org")
    ...
    get<Routes.SlackChannelsToPickForSync> { params ->
        runAuthorized { spaceTokenInfo ->
            val response =
                SlackChannelsToPickForSync(spaceTokenInfo).getSlackChannelsToPickFrom(
...
private suspend fun getSpaceTokenInfo(spaceUserToken: String): SpaceTokenInfo? {
    ...
    val spaceAppInstance =
        jwtClaimsSet.audience.singleOrNull()?.let { db.spaceAppInstances.getById(it) } ?: return null
    val spaceUserId = jwtClaimsSet.subject
    return SpaceTokenInfo(spaceAppInstance, spaceUserId, spaceUserToken)
}

private val slackAppPermissionScopes = listOf(
    "users.profile:read",
    "users:read",
    "users:read.email",
    "channels:read",
...
@Serializable
data class SlackWorkspacesResponse(val workspaces: List<SlackWorkspace>, val canManage: Boolean)

@Serializable
data class SlackWorkspace(val id: String, val domain: String, val name: String)
